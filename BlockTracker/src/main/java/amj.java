...
public final class amj {
   ...
   public boolean a(ahd var1, aqu var2, dt var3, ej var4, float var5, float var6, float var7) {
	   ...
	   //TODO
	   //The DT is the coordinate of the clicked on block,
	   //not of where the block will be.
	   //Find where the block would have otherwise gone to if I didn't return false.
	   
	   
		//BlockTrackerTool code BEGIN
		if (BlockTrackerTool.isPlayerTooled(var1.d_())) { //Checks if the tool is enabled.
			if (!(var1.bg.h() == null)) { //Makes sure they are holding something.
				if (var1.bg.h().b().a().equals("tile.log")) { //Checks the item held is a Log block.
					BlockTrackerTool.getCoordEdits(var1, var3, "BlockPlace"); //Get edits to the coords.
					return false;
				}
			}
		}
